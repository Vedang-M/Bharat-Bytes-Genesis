...
const WaterStatusScreen = () => {
  ...
  const fetchWaterStatus = async (lat, lon) => {
    ...
    try {
      ...
      // Parse location string (format: "City, District, State")
  ...
  // Derive status from percentage with specified thresholds
  ...
  const waterPct = displayWaterData.waterPercentage || 0;
  const derivedStatus = getStatusFromPercentage(waterPct);
  const current = statusConfig[derivedStatus] || statusConfig.critical;
      ...
      <div className="relative z-10 max-w-md lg:max-w-5xl mx-auto px-5 pt-4 pb-40 md:pb-10 md:pt-24 flex flex-col flex-1 w-full gap-6 md:h-full md:overflow-hidden md:justify-between">
        ...
        >
          <div className="flex flex-col justify-center flex-1">
            <span className="text-sm md:text-xs font-bold text-white/70 uppercase tracking-wider mb-0.5">
              {t.header}
            </span>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className="
                  ml-1 md:ml-2 flex items-center gap-2 px-4 py-2.5 md:px-3 md:py-1.5 rounded-full
                  bg-white/20 hover:bg-white/30 border border-white/30
                  text-sm md:text-xs font-bold text-white
                  transition-all active:scale-95
                  disabled:opacity-50 disabled:cursor-not-allowed
                "
              >
                <MapPinned size={16} className="md:w-[14px] md:h-[14px]" />
                {isLoadingLocation ? t.fetchingLocation : t.fetchLocation}
                {isLoadingLocation && (
                  <span className="h-4 w-4 md:h-3 md:w-3 animate-spin rounded-full border-2 border-white border-t-transparent" />
                )}
              </button>
            </div>
          </div>
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 items-center min-h-0">
          ...
          {/* RIGHT SECTION */}
          <div className="flex flex-col gap-6 justify-center h-full pb-16 md:pb-0">
